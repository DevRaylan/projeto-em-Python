...
class SistemaAcademicoApp:
    def __init__(self, root):
        ...
        self.label_arquivo_salvo = tk.Label(self.root, text="Arquivo não salvo ainda.", fg="blue")
        self.label_arquivo_salvo.grid(row=1, column=0, columnspan=2, pady=10)

        # Botão para abrir o local onde o arquivo está salvo
        self.botao_abrir_local = tk.Button(self.root, text="Abrir Local do arquivo", command=self.abrir_local, state=tk.DISABLED)
        self.botao_abrir_local.grid(row=2, column=0, columnspan=2)
    ...
    def adicionar_professor(self):
        ...
        # Gerar o nome do arquivo com base nas informações do professor
        nome_arquivo = f"Relatorio {disciplina} - {codigo_disciplina} - {ano}.txt"

        # Pedir ao usuário para escolher onde salvar o arquivo
        caminho = filedialog.asksaveasfilename(defaultextension=".txt", filetypes=[("Arquivos de Texto", "*.txt")], initialfile=nome_arquivo)
        if not caminho:
            messagebox.showwarning("Caminho não escolhido", "Você precisa escolher um local para salvar o arquivo!")
            return
        self.caminho_arquivo = caminho
        self.label_arquivo_salvo.config(text=f"Arquivo salvo em: {self.caminho_arquivo}")
        self.botao_abrir_local.config(state=tk.NORMAL)
    ...
    def abrir_local(self):
        if self.caminho_arquivo:
            ...
            os.startfile(pasta)  # No Windows, abre o explorador de arquivos na pasta
